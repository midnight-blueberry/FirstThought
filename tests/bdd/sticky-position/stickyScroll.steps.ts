...
defineFeature(feature, (test: any) => {
  test('Сохраняет смещение прокрутки после смены темы', ({ given, when, then }: any) => {
    let scrollRef: any = null;
    let ctx: ReturnType<typeof getStickySelectionContext> = null;
    ...
    given('зафиксирован press по "theme:dark" с измерениями y=190 и h=20', async () => {
      const pressedRef = {
        current: {
    ...
    given('текущая прокрутка равна 150', () => {
    ...
    when('я применяю изменения со StickySelection', async () => {
      await act(async () => {
