...
export const UIManager = {
  measureLayout: (_node: any, _relativeTo: any, _onFail: () => void, onSuccess: (x: number, y: number) => void) => {
    if (onSuccess) {
      onSuccess(0, 0);
    }
  },
};
export const findNodeHandle = (node: any) => node ?? 0;
export const ScrollView = () => null as any;
