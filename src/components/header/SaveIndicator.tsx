...
export const SaveIndicatorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const controllerRef = useRef<ReturnType<typeof createSaveIndicatorController> | null>(null);

  const controller = useMemo(() => {
    if (!controllerRef.current) {
      controllerRef.current = createSaveIndicatorController({ setVisible, opacity });
    }
  ...
  const value = useMemo(
    () => ({
      showFor2s: controller.showFor2s,
      hide: controller.hide,
      opacity,
      visible,
    }),
    [controller.hide, controller.showFor2s, opacity, visible],
  );
